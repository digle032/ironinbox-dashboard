...
| `src/pages/Settings.tsx` | Includes theme switching, alert behavior toggles, risk sensitivity sliders, data retention options, mock role preview, and engagement tracking display. |
| `src/pages/Login.tsx` | Provides the demo authentication screen for sign-in and sign-up. |
| `src/pages/Account.tsx` | Displays user profile information, manages connected integrations (Gmail, Outlook, Slack), and allows disconnecting or adding new monitored accounts. |
...
The account page now displays the signed-in user's profile information alongside a dynamic list of connected integrations (Gmail, Outlook, and Slack). Users can view the connection status of each integration, disconnect specific integrations (which removes inbox and flagged data for that source from the current session), and navigate to add another monitored account. Disconnecting every integration signs the user out automatically.

The settings area has been expanded to include a comprehensive suite of controls. Users can toggle between light and dark themes, configure alert behaviors (dashboard alerts, critical risk email notifications, and flagged queue highlighting), adjust a risk sensitivity slider to set the score threshold for flagging emails, and select mock data retention policies (7 days, 30 days, or until user deletes). It also features a browser-based mock role preview for viewer, manager, and admin roles, and an engagement tracker panel that records recent visits in browser storage and is visible to all roles on the Settings page.
